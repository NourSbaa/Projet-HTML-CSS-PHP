...
<div class="box">
  ...
  <input type="submit" name="supprimer" value="supprimer"> 
        ...
        <?php
           ...
           $sql ="DELETE FROM `ch` WHERE id like '$id' ;";           
         
     if($conn -> query($sql) === TRUE){      //si la suppression avec sucès alors :
        header("Location:affichage2.php") ;//Si le nombre de ligne est > 0 , on sera redirigé vers la page bienvenu
    } else {      //sinon
        header("Location:supprimer3.php") ;//Si le nombre de ligne est > 0 , on sera redirigé vers la page bienvenu
    }
        }
