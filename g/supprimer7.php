...
<div class="box">
  ...
  <form   method="POST"  action="supprimer7.php">
  <fieldset>
  <h2> suppression</h2>
  ...
  <input type="submit" name="supprimer" value="supprimer"> 
     ...
     if($conn -> query($sql) === TRUE){      //si la suppression avec sucès alors :
        header("Location:affichage6.php") ;//Si le nombre de ligne est > 0 , on sera redirigé vers la page bienvenu
    } else {      //sinon
        header("Location:supprimer7.php") ;//Si le nombre de ligne est > 0 , on sera redirigé vers la page bienvenu
    }
        }
