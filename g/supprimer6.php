...
<div class="box">
  ...
  <form   method="POST"  action="supprimer6.php">
  <fieldset>
  <h2> suppression</h2>
  ...
  <input type="submit" name="supprimer" value="supprimer"> 
        ...
        <?php
           ...
           $sql ="DELETE FROM `ag` WHERE id like '$id' ;";           
         
     if($conn -> query($sql) === TRUE){      //si la suppression avec sucès alors :
        header("Location:affichage5.php") ;//Si le nombre de ligne est > 0 , on sera redirigé vers la page bienvenu
    } else {      //sinon
        header("Location:supprimer6.php") ;//Si le nombre de ligne est > 0 , on sera redirigé vers la page bienvenu
    }
        }
